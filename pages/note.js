...
function Note(){
    ...
    useEffect(() => {
        ...
        if(pageMode === 'add')
            ...
            dispatchSrc({
                type: 'SRC_TYPE_DROPDOWN_ON_CHANGE',
                id: router.query.srcTypeId,
                hasName: 'srcName' in router.query,
                hasURL: 'srcURL' in router.query,
            });

            if('srcName' in router.query)
                dispatchSrc({
                    type: 'SRC_NAME_INPUT_ON_CHANGE',
                    name: router.query.srcName,
                });

            if('srcURL' in router.query)
                dispatchSrc({
                    type: 'SRC_URL_INPUT_ON_CHANGE',
                    url: router.query.srcURL,
                });
